...
const recordNode = document.getElementById("best-score");
const restartclick = document.getElementById("restart");
...
let count = 0; // счётчик смены направления змейки

document.addEventListener("keydown", direction);

function direction(event) {
  if (count === 0) {
    // если змейка не меняла направление
...
class Snake {
  constructor(canvas, box, fieldSize) {
    ...
    this.position = [
      ...
      {
        x: Math.floor(this.fieldSize / 2),
        y: Math.floor(this.fieldSize / 2) + 1,
      },
    ];
    this.color = "#5d9700";
  }
  ...
  _biteSelf() {
    ...
    for (let i = 1; i < this.position.length; i++) {
      if (JSON.stringify(this.position[i]) === JSON.stringify(snakeHead)) {
        endGame = true;
        restartclick.classList.add("visible");
        if (score > record) {
          record = score;
          recordNode.textContent = record;
  ...
  draw() {
    ...
    let snakeBox = this.box - 2;
    this.position.forEach((el) => {
      let x = el["x"] * this.box + 1;
